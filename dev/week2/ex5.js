...
class WallLightManager {
  ...
  initWallLights() {
    ...
    const fbBaseLight = new THREE.RectAreaLight(color.white, intensity, roomWidth, roomHeight)

    this.backWallLight = fbBaseLight.clone()
    this.backWallLight.name = 'backWallLight'
    this.backWallLight.position.set(0, roomHeight / 2, -(roomDepth / 2 - lightMergin))
    ...
    this.frontWallLight.position.set(0, roomHeight / 2, roomDepth / 2 - lightMergin)
    this.scene.add(this.frontWallLight)

    const rlBaseLight = new THREE.RectAreaLight(color.white, intensity, roomDepth, roomHeight)

    this.leftWallLight = rlBaseLight.clone()
    this.leftWallLight.name = 'leftWallLight'
    this.leftWallLight.position.set(-roomParams.roomWidth / 2 + lightMergin, roomHeight / 2, 0)
    ...
    this.rightWallLight.position.set(roomWidth / 2 - lightMergin, roomHeight / 2, 0)
  ...
  updateWallLightColor() {
    if (!this.backWallLight) throw new Error('backWallLight is not initialized')
...
const main = () => {
  ...
  const wallLightManager = new WallLightManager(scene)

  initGUI(scene, wallLightManager)
